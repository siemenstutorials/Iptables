...
echo -e "${red}用途${black}: 设置iptables端口转发"
echo -e "${red}注意${black}: IP转发VPS重启后转发失效；域名转发VPS重启后依然有效；转发生效时间2分钟"
echo
setupService(){
    wget -qO /usr/local/bin/dnat.sh https://raw.githubusercontent.com/arloor/iptablesUtils/master/dnat.sh||{
        echo "欢迎订阅我的YouTube频道-SiemensTutorials"
        exit 1
    }
    echo 
...
echo  -e "${red}请选择你的操作选项 [Ctrl+C 退出]${black}"
